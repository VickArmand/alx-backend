#!/usr/bin/python3
"""4-mru_cache module has a class MRUCache
...
class MRUCache(BaseCaching):
    ...
    def put(self, key, item):
        ...
        you must discard the least recently used item (MRU algorithm)
        ...
        if key is not None and item is not None:
            ...
            if size == self.MAX_ITEMS and key not in existing_keys:
                mostRecentlyUsed = self.recentlyUsed[-1]
                self.cache_data.pop(mostRecentlyUsed)
                self.recentlyUsed.remove(mostRecentlyUsed)
                print(f"DISCARD: {mostRecentlyUsed}")
